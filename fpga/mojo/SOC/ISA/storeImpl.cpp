...
void StoreInstructionImpl::parseFlags() {
...
void StoreInstructionImpl::parseOpcode() {
    mEncoded  = opcodes[Instruction::STORE] << 28;
    parseFlags();
}

void StoreInstructionImpl::validate() {
    if (mLOperand.type != OperandType::REGISTER) {
        throw std::runtime_error("validate: store instruction must have a register as a left operand");
    }
    if (mROperand.type != OperandType::REFERENCE_REGISTER
            && mROperand.type != OperandType::REFERENCE_REGISTER_OFFSET) {
        throw std::runtime_error("validate: store instruction must have an immediate or a reference  as a right operand");
    }
}

void StoreInstructionImpl::encode() {
